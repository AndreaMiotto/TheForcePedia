...
class StarshipDetailsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        persons = starship.pilots?.allObjects as! [Person]
        films = starship.films?.allObjects as! [Film]
    ...
    func buildCellForDetails(withRowIndex index: Int) -> UITableViewCell {
        ...
        case 0:
            ...
            if let s_class = starship.starship_class {
        ...
        case 2:
            cell.textLabel?.text = "MGLT:"
            //if the person has a connection with an homeworld
            if let mglt = starship.mglt {
                cell.detailTextLabel?.text = mglt
            } else {
                cell.detailTextLabel?.text = "unknown"
            }
        case 3:
            cell.textLabel?.text = "Atm Speed:"
            if starship.max_atmosphering_speed != 0 {
                cell.detailTextLabel?.text = "\(starship.max_atmosphering_speed)"
            } else {
                cell.detailTextLabel?.text = "unknown"
            }
        case 4:
            ...
            if starship.cost_in_credits != 0 {
                cell.detailTextLabel?.text = "\(starship.cost_in_credits) credits"
            } else {
                cell.detailTextLabel?.text = "unknown"
            }
        case 5:
            cell.textLabel?.text = "Hyperdrive:"
            if starship.hyperdrive_rating != 0 {
                cell.detailTextLabel?.text = "\(starship.hyperdrive_rating) ratio"
            } else {
                cell.detailTextLabel?.text = "unknown"
            }
        case 6:
            ...
            if starship.length != 0 {
                cell.detailTextLabel?.text = "\(starship.length) mt."
            } else {
                cell.detailTextLabel?.text = "unknown"
            }
        case 7:
            ...
            if starship.crew != 0 {
                cell.detailTextLabel?.text = "\(starship.crew)"
            } else {
                cell.detailTextLabel?.text = "unknown"
            }
        case 8:
            ...
            if starship.passengers != 0 {
                cell.detailTextLabel?.text = "\(starship.passengers)"
            } else {
                cell.detailTextLabel?.text = "unknown"
            }
        case 9:
            cell.textLabel?.text = "Stocks:"
            if let consumables = starship.consumables  {
    ...
    func updateConnections() {
        self.createPersonsConnection(fromStarship: starship, toPersons: starship.pilot_urls)
        self.createFilmsConnection(fromStarship: starship, toFilms: starship.film_urls)
        tableView.reloadData()
    }
    
    ///make the connections between the starship and the persons
    func createPersonsConnection(fromStarship starship: Starship, toPersons persons: [String]?) {
        ...
        for url in urls {
            ...
            if let existingPerson = fetchedPersons?.first {
                ...
                starship.pilots?.adding(existingPerson)
    ...
    ///make the connections between the starship and the films
    func createFilmsConnection(fromStarship starship: Starship, toFilms films: [String]?) {
        ...
        for url in urls {
            ...
            if let existingFilm = fetchedFilms?.first {
                //Yes, make the connection
                starship.films?.adding(existingFilm)
    ...
    @IBAction func searchImages(_ sender: UIBarButtonItem) {
        guard  let name = starship.name else {
            return
        }
